    ...
    <vl-region>
      <vl-layout>
        <vl-grid>
          ...
          <vl-column width="3">
            <vl-side-navigation
              class="example-sticky"
              title="Zie ook"
            >
              <vl-side-navigation-list>
                <vl-side-navigation-item
                  href="https://test.data.vlaanderen.be/shacl-validator/"
                  text="OSLO Validator"
                />
                <vl-side-navigation-item
        ...
        <vl-grid mod-stacked>
          <vl-column width="5">
            <vl-action-group>
              ...
              <vl-button
                v-else
                id="parseButtonInactive"
                @click="setAction('parsing')"
              >
                JSON-LD Parsing
              </vl-button>
              <vl-button
                v-if="action === 'shacl'"
                id="shaclButtonActive"
                mod-tertiary
              >
                SHACL validatie
              </vl-button>
              <vl-button
                v-else
                id="shaclButtonInactive"
                @click="setAction('shacl')"
              >
                SHACL validatie
              </vl-button>
            </vl-action-group>
          ...
          <vl-column width="6">
            <vl-action-group>
              ...
              <vl-input-field
                v-model="documentURL"
                style="width: 91%;"
                :class="{'vl-input-field--error': fetchError}"
              ...
              <p
                ...
                style="color: red;"
              >
                Fout bij het ophalen van de URL
              </p>
            </vl-action-group>
          </vl-column>
          <vl-column v-if="action === 'parsing'">
            <ParseComponent :document-data="documentData" />
          </vl-column>
          <vl-column v-if="action === 'shacl'">
            <ShaclComponent :document-data="documentData" />
          </vl-column>
        </vl-grid>
      </vl-layout>
    </vl-region>
        ...
        async fetchDocument() {
            ...
            if (this.documentURL != '') {
                this.documentData = await new Promise(resolve => {
                    fetch(this.documentURL).then(result => {
